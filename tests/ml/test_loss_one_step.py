...
Uses SmoothedBCEWithLogitsLoss with label_smoothing=0.1.
...
class TestOneTrainingStep:
    def test_loss_backward_completes(self):
        ...
        model.train()
        loss_fn = SmoothedBCEWithLogitsLoss(pos_weight=1.5, smoothing=0.1)
    ...
    def test_loss_value_range(self):
        ...
        assert loss.item() >= 0.0, "BCE loss must be non-negative"

    def test_label_smoothing_effect(self):
        """Smoothed labels should be pulled toward 0.5."""
        from ml_pipeline.training.losses import SmoothedBCEWithLogitsLoss

        loss_fn = SmoothedBCEWithLogitsLoss(pos_weight=1.0, smoothing=0.1)
        logits = torch.ones(4, 1) * 2.0

        hard_labels = torch.ones(4, 1)
        loss_hard = loss_fn(logits, hard_labels)

        loss_fn_no_smooth = SmoothedBCEWithLogitsLoss(pos_weight=1.0, smoothing=0.0)
