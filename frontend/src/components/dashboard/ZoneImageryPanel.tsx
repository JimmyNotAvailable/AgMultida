...
export function ZoneImageryPanel({ latest, history, mode, isLoading, isError, onModeChange }: ZoneImageryPanelProps) {
  ...
  const statusLabel = isLoading ? 'Loading' : isError ? 'Unavailable' : latest?.stale ? 'Stale' : 'Fresh'
  const statusClass = isError ? 'critical' : latest?.stale ? 'warning' : 'healthy'
  ...
  return (
    <article className="data-card imagery-panel" data-testid="imagery-panel">
      ...
      {!isLoading && !isError && !previewUrl ? <p className="section-copy">No preview available.</p> : null}
